...
| `xf86bigfont` resurrection & cleanup | Step 1 (make it build everywhere) in progress — CI default-on surfaced bit-rot on 7 non-Linux lanes | `BIGFONT.md`; PRs #3202 (enable-in-CI), #3201 (pagesize/CSRG), branch `xlibre/bigfont-consolidate-cleanup` (ResetProc drop, pending) | Root causes: `sys/sysmacros.h` (5 BSD/macOS), `dix.h` `ARRAY_SIZE` redef (solaris), `geteuid`/`getegid`+unused-var (mingw). BSD/macOS compile-break → backport candidate. Later ideas in Parkplatz. **Competing proposal in flight, see next row:** maintainer also wants a draft PR to remove `xf86bigfont` entirely — the two are deliberately parallel options to put before the community, not a contradiction to resolve here |
...
| Propose removing XFS (X Font Server client) support | Not started — planned: open a **draft PR**, then bring to community discussion | none yet | Maintainer request, 2026-07-01, parallel to the bigfont-removal draft above. Same pattern: draft-PR-first so there's something concrete to discuss, not just an RFC. Scope not yet mapped — next step is locating the XFS client code (font-server protocol support, historically `os/*` + the `fs` font-catalogue backend) before drafting |
