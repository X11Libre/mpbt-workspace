...
| `scripts/show-branch-file <ref> <path> [symbol]` | print a repo file (or symbol region) at any ref via the GitHub API — for backport applicability checks; auto-handles the `Xext/<ext>/` ↔ `<ext>/` reorg. Also available (preferred) as `scripts/starfleetctl show-branch-file` — see its row below |
| `scripts/starfleetctl backport-applies <master-path> <grep-ERE> [release ...]` | run the applicability grep across all release lines at once (wraps `show-branch-file`) — classify each branch vulnerable / already-fixed / N-A in one command. **Bash original removed 2026-07-06** — no remaining `scripts/*` dependents, parity already proven |
...
| `scripts/starfleetctl pr-view <pr#> [json-fields]` | print PR metadata via `gh pr view --json <fields>` as one canonical, allowlistable command (default fields `number,title,state`) — exists so agents don't chain it after an unrelated `echo`/`export` (breaks `Bash(gh *)` prefix matching, see the AGENT_ID-export gotcha below); also covers the backport workflow's step-0 "already backported?" body check. **Bash original removed 2026-07-06** — no remaining `scripts/*` dependents, parity already proven |
...
| `scripts/starfleetctl pr-ci <pr#\|URL>` \| `--json` | **quick CI status of a PR** — one canonical command (give it a PR number *or* full PR URL, nothing else) so agents don't hand-assemble `gh pr checks`/`gh pr view --json statusCheckRollup` combos. Classifies every check **by conclusion, not count** (fail-fast means a big red number is usually a few `FAILURE` + many collateral `CANCELLED`): prints header + bucket counts (pass/FAIL/cancelled/pending/skip), lists the real failures + pending checks, a one-line verdict, and a known-flake hint (BSD/Solaris VM, xephyr-glamor timeout, go-xts) when the reds match. For the failure *logs*, use `pr-job-logs`. Pre-authorized in `.claude/settings.json`. **Bash original removed 2026-07-06** — no remaining `scripts/*` dependents, parity already proven |
...
back to `user@host` if unset, same as `scripts/agent-bus`):
