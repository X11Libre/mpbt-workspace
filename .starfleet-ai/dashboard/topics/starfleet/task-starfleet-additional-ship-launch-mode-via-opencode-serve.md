...
- [x] The existing PTY mode functions identically to its current state when the API flag is disabled.
- [x] Launching multiple ships concurrently works flawlessly using separate socket files inside `.starfleetctl/var/ships/`.
- [x] Stale `.sock` files are automatically cleaned up when a ship process exits.
- [x] Selecting an API-mode ship in the frontend displays a structured native chat/log UI instead of an empty or broken terminal.
- [x] Message injection and custom model-routing function correctly in both modes over the Unix socket layer.
- [x] All code compiles without errors (`go build ./...`) and passes automated tests.
...
- 2026-09-17T14:30:00Z Scotty: **IMPLEMENTIERT** in branch `starfleetctl-api-mode` (worktree `_WORK_/starfleetctl/sources/starfleetctl-api-mode`):
  - `internal/opencode/` Package mit `Client` (UDS-HTTP-Transport, Session/Model/Chat/Message APIs) und `Server` (headless `opencode serve` Management, Socket-Cleanup)
  - `session.LaunchShip` mit `--mode=api|pty` Flag; API-Mode startet `opencode serve --socket <path>` statt termctl
  - `StopShip` bereinigt `opencode.sock` (stale cleanup)
  - Web-UI: Launch-Form mit `ns_native_api` Checkbox → sendet `mode` an `/api/ship`
  - `apiShipLaunch` übergibt `Mode` an `session.LaunchShipOpts`
  - Model-Router/Message-Injection: funktioniert unverändert (opencode nutzt generierten Config mit model-proxy Providers; `SendMessage`/`SendMessageStream` für A2A-Injection über UDS)
  - Build: `make` in worktree läuft sauber durch (alle Tests grün)
