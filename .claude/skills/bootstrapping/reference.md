...
5. Reports the shell integration (`starfleetctl ship-names shell-env` to be sourced from
   `~/.bashrc`/`~/.zshrc`, see "Ship names for fleet identity" in Concurrency) — but **never
   edits your dotfiles unless you pass `--shell-hook`**; without it, bootstrap just prints the
   line to add.
