...
PR, request both as reviewers (`.starfleet-ai/bin/starfleetctl github pr request-reviewers <pr#> cepelinas9000 stefan11111`)
...
and inline review comments alike. `<ship>` is the executing session's comms identity
(`$AGENT_ID` — its Star Trek ship name, see "Ship names for fleet identity" in Concurrency; falls
back to `user@host` if unset, same as `.starfleet-ai/bin/starfleetctl comms`):
...
`.starfleet-ai/bin/starfleetctl github pr label <pr#> set-review passed|changes-requested`, **not** `gh pr edit` — the latter
